...
(function () {
    ...
    function calcBookSize() {
        ...
        if (isMob) {
            return { w: vw, h: Math.max(300, Math.round(availH * 0.97)), mode: 'mobile' };
        }
        const isTab = vw <= TABLET_BP && ('ontouchstart' in window || navigator.maxTouchPoints > 0);
        if (isTab) {
            return { w: Math.round(vw * 0.94), h: Math.max(400, Math.round(availH * 0.94)), mode: 'tablet' };
        }
        const w = Math.min(BOOK_MAX_W, vw - 80);
        return { w, h: Math.max(400, Math.round(availH * BOOK_RATIO)), mode: 'desktop' };
    }
